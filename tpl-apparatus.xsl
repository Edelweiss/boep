   ...
   <xsl:template name="ddbdp-app">
      ...
      <xsl:choose>
         <xsl:when test="not(ancestor::t:choice or ancestor::t:subst or ancestor::t:app or
            ...
            <!--<xsl:call-template name="lbrk-app"/>--><!-- cl: get rid of linebreak -->
            <!-- in htm-tpl-apparatus.xsl or txt-tpl-apparatus.xsl -->
            <!--<xsl:call-template name="app-link">
               <xsl:with-param name="location" select="'apparatus'"/>
            </xsl:call-template>--><!-- cl: get rid of backlink -->
         ...
         <xsl:choose>
            <xsl:when test="local-name()=('choice','subst','app')">
              ...
              <xsl:attribute name="title"><xsl:apply-templates select="$title" mode="sqbrackets"/></xsl:attribute>
               ...
               <!-- cl: copied, swapped and modified the following two lines -->               
               ...
               <xsl:text> </xsl:text><xsl:value-of select="normalize-space(replace(replace($part3, 'FNORD(\S)*', ''), '\(\*\)', ''))"/>
               <xsl:if test="string(normalize-space($part1))">
                 <xsl:text> </xsl:text>
               </xsl:if>
               <xsl:value-of select="normalize-space(replace(replace($part1, 'FNORD(\S)*', ''), '\(\*\)', ''))"/>
 ...
 -->
   ...
   <xsl:template name="appchoice">
     ...
     <!-- {{ (<xsl:value-of select="$path/*" />) 
      ...
      <xsl:choose>
         ...
         <xsl:when test="$apptype=('siccorr')">
            <!-- **CORR - <xsl:value-of select="$path/t:corr/node()"/>** -->
            <xsl:text> (l. </xsl:text><!-- cl: added space and opening bracket -->
            <xsl:apply-templates select="$path/t:corr/node()"/>
            <!-- xsl:text> (corr)</xsl:text --><!-- cl: supress (corr) -->
            <xsl:text>)</xsl:text><!-- cl: added closing bracket -->
         </xsl:when>
         <!-- *SUBST* -->
         <xsl:when test="$apptype='subst'">
            <!--**SUBST - <xsl:value-of select="$path/t:del/node()"/>/<xsl:value-of select="$path/t:add/node()"/>** -->       
         ...
         <xsl:when test="$apptype=('appbl','apppn','apped')">
            ...
            <xsl:choose>
               ...
               <xsl:when test="starts-with($path/t:lem/@resp,'PN ')">
                  ...
                  <xsl:text> (via PN)</xsl:text-->
               </xsl:when><!-- cl: get rid of "Author ( via PN)" when treating »lem« stuff -->
         ...
         <xsl:otherwise><!-- RDG OMISSION -->
            <xsl:call-template name="wordBefore"> <!-- cl: handle omissions -->
               <xsl:with-param name="fix" select="." />
            </xsl:call-template>
            <!--xsl:text> ex om. </xsl:text-->
            <xsl:call-template name="wordAfter">
               <xsl:with-param name="fix" select="." />
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>

      <xsl:choose> <!-- cl: insert processing instruction for TEI:lem -->
         <xsl:when test="not(string(normalize-space(t:lem))) and not(t:lem/t:gap) and not(t:lem/t:g)">
            <xsl:text> → </xsl:text> <!-- cl: insert → -->
            <xsl:text> Om.</xsl:text>
         </xsl:when>
         <xsl:when test="$child">
            <xsl:text> → </xsl:text> <!-- cl: insert → -->
            <xsl:if test="not(t:*[local-name()=('reg','corr','del','rdg')]/node())">
               <xsl:call-template name="wordBefore">
                  <xsl:with-param name="fix" select="." />
               </xsl:call-template>
            </xsl:if>
            <xsl:apply-templates select="t:lem/node()"/>
            <xsl:if test="not(t:*[local-name()=('reg','corr','del','rdg')]/node())">
               <xsl:call-template name="wordAfter">
                  <xsl:with-param name="fix" select="." />
               </xsl:call-template>
   ...
   <xsl:template name="app-ed-mult-with-param">
      ...
      <xsl:choose>
         ...
         </xsl:otherwise>--><!-- cl: get rid of »prev. ed.«-->
      </xsl:choose>
      <xsl:if test="position()!=last()">
         <xsl:text> : </xsl:text>
      </xsl:if>
